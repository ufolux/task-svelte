<script>
  ...
  import tasks from 'svelte-awesome/icons/tasks';
...
<aside class="sidebar border-color-sidebar-border bg-color-sidebar-bg flex w-40 flex-col border-r">
  <div class="sidebar-logo flex h-15 items-center justify-center gap-2">
  ...
  <div class="flex h-full flex-col justify-between">
    ...
    <ul class="sidebar-content flex flex-col px-2 py-4">
      ...
      <li class="sidebar-item hover:bg-color-hover mt-auto flex items-center gap-2 rounded-md p-2">
        <button onclick={toggleTheme} class="text-primary">
          <Icon data={getTheme() === 'light' ? moon : sun} />
          <span>{getTheme() === 'light' ? 'Dark Mode' : 'Light Mode'}</span>
        </button>
      </li>
    </ul>
  </div>
